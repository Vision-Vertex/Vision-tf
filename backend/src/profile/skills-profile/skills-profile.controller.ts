...
import { ApiTags, ApiBearerAuth, ApiOperation, ApiOkResponse, ApiBadRequestResponse, ApiUnauthorizedResponse, ApiForbiddenResponse, ApiParam } from '@nestjs/swagger';
...
export class SkillsProfileController {
  ...
  @ApiOperation({
    ...
    description: 'Adds a single skill to the developer\'s profile. Skill must not already exist.',
  })
  @ApiOkResponse({ 
  ...
  @ApiOperation({
    ...
    description: 'Removes a specific skill from the developer\'s profile. If skill does not exist, operation completes successfully.',
  })
  @ApiParam({
    name: 'skill',
    description: 'The skill name to remove (URL encoded if contains special characters)',
    example: 'JavaScript',
    type: 'string'
  })
  @ApiOkResponse({ 
